...
// blocks look presentable out of the box.
function BlockFrame({ id, pageKey, children, full, style }) {
...
export function Hero({ id, pageKey, props }) {
  const p = props || {};
  return (
    <BlockFrame id={id} pageKey={pageKey} style={{
...
export function CTA({ id, pageKey, props }) {
  const p = props || {};
  return (
    <BlockFrame id={id} pageKey={pageKey} style={{
...
export function FeatureGrid({ id, pageKey, props }) {
  const p = props || {};
  const items = Array.isArray(p.items) ? p.items : [];
  return (
    <BlockFrame id={id} pageKey={pageKey}>
      {p.eyebrow && (
        <div style={{
          textAlign: 'center',
          fontSize: 'var(--tapas-eyebrow-size, 11px)',
...
export function Footer({ id, pageKey, props }) {
  const p = props || {};
...
export function TextImage({ id, pageKey, props }) {
  const p = props || {};
...
export function Testimonials({ id, pageKey, props }) {
  const p = props || {};
  const items = Array.isArray(p.items) ? p.items : [];
  return (
    <BlockFrame id={id} pageKey={pageKey} style={{
...
export function Pricing({ id, pageKey, props }) {
  const p = props || {};
...
export function FAQ({ id, pageKey, props }) {
  const p = props || {};
  const items = Array.isArray(p.items) ? p.items : [];
...
export function Gallery({ id, pageKey, props }) {
  const p = props || {};
...
export function Newsletter({ id, pageKey, props }) {
  const p = props || {};
  return (
    <BlockFrame id={id} pageKey={pageKey} style={{
...
export function BookList({ id, pageKey, props }) {
  const p = props || {};
...
export function BlogList({ id, pageKey, props }) {
  const p = props || {};
...
export function EventList({ id, pageKey, props }) {
  const p = props || {};
  const limit = Math.max(1, Math.min(24, Number(p.limit) || 6));
