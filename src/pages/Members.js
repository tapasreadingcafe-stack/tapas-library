    ...
    <div className="page-content">
      ...
      <div className="page-header">
        ...
        <div className="header-actions">
          <button className="btn" onClick={() => { setPickerSearch(''); setShowMemberPicker(true); }}
      ...
      {/* Quick member picker — search + jump to a member's profile */}
      {showMemberPicker && (
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ overflowY: 'auto', padding: '0 8px 12px' }}>
              {(() => {
                ...
                const list = members.filter(m => !q || (m.name || '').toLowerCase().includes(q) || (m.phone || '').includes(q)).slice(0, 50);
                if (list.length === 0) return <div style={{ padding: '20px', textAlign: 'center', color: '#999', fontSize: '13px' }}>No members found</div>;
                return list.map(m => (
                  <button key={m.id} onClick={() => { setShowMemberPicker(false); navigate(`/member/${m.id}`); }}
...
function ActionIcon({ name, size = 18 }) {
  ...
  switch (name) {
    ...
    case 'renew':
      // Two thick circular arrows — matches user's reference image.
      return (
        <svg viewBox="0 0 512 512" style={common} aria-hidden="true">
          <path d="M256 64C150 64 64 150 64 256c0 36 10 70 28 99l44-44c-9-17-14-35-14-55 0-73 60-133 134-133 22 0 43 5 61 15l-31 31h97V72L355 105c-29-26-66-41-99-41zm192 109l-44 44c9 17 14 35 14 55 0 73-60 133-134 133-22 0-43-5-61-15l31-31H157v97l28-33c29 26 66 41 99 41 106 0 192-86 192-192 0-36-10-70-28-99z"/>
        </svg>
      );
    case 'email':
      return (
    ...
    case 'phone':
      // WhatsApp speech-bubble + handset — matches user's reference.
      return (
        <svg viewBox="0 0 32 32" style={common} aria-hidden="true">
          <path d="M16.001 3C9.373 3 4 8.373 4 15a11.94 11.94 0 0 0 1.785 6.298L4 29l7.93-1.74A11.94 11.94 0 0 0 16.001 27C22.628 27 28 21.628 28 15S22.628 3 16.001 3zm6.572 17.121c-.27.762-1.563 1.46-2.183 1.51-.582.05-1.32.07-2.13-.135-.49-.124-1.122-.34-1.93-.69-3.4-1.466-5.617-4.89-5.787-5.118-.17-.227-1.39-1.85-1.39-3.532 0-1.682.882-2.51 1.196-2.853.314-.343.68-.43.91-.43h.654c.21 0 .49-.078.766.585.282.682.96 2.357 1.045 2.527.085.17.142.37.028.595-.114.227-.17.366-.34.563-.17.197-.358.44-.51.59-.17.17-.346.353-.15.694.197.343.875 1.444 1.88 2.34 1.293 1.152 2.385 1.512 2.726 1.683.343.17.543.142.745-.085.198-.227.852-.99 1.082-1.33.227-.34.453-.282.766-.17.314.114 2 .945 2.342 1.116.343.17.572.255.66.397.085.142.085.83-.183 1.59z"/>
        </svg>
      );
    case 'delete':
      // Thick bold X — matches user's reference image.
      return (
        <svg viewBox="0 0 24 24" style={common} aria-hidden="true">
